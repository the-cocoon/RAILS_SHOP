#= require jquery2
#= require jquery-ui
#= require jquery_ujs
#= require turbolinks

#= require jquery-ui/core
#= require jquery-ui/autocomplete

#= require log_js
#= require jquery.data-role-selector

#= require notifications/vendors/toastr
#= require notifications

#= require crop_tool/crop_tool
#= require crop_tool/jcrop/jquery.Jcrop

#= require custom_select/custom_select

#= require components/jQuery-File-Upload/jquery.iframe-transport
#= require components/jQuery-File-Upload/jquery.fileupload

#= require rails_shop/backend/products_edit
#= require rails_shop/backend/delivery_types
#= require rails_shop/backend/editable_block_switcher
#= require rails_shop/backend/order_payment_button
#= require rails_shop/backend/order_payment_form
#= require rails_shop/backend/shop_search_form

#= require rails_shop/backend/shop_category_rels

#= require components/ckeditor/init

#= require user_room/registration_accordion
#= require user_room/section-switcher
#= require user_room/login_popup

#= require ptz-tabs/ptz-tabs
#= require attached_images/attached_images

#= require JODY/base
#= require jody_notificator_init

#= require the_sortable_tree/jquery.ui.nestedSortable
#= require the_sortable_tree/sortable_ui/base

#= require rails_shop/backend/shop_spoiler
#= require shop_filters/shop_filter_form

#= require rails_shop/backend/application_initializer

